...
import deductionListExporterFields from '@/modules/deduction/deduction-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !deductionListExporterFields ||
          !deductionListExporterFields.length
        ) {
          throw new Error(
            'deductionListExporterFields is required',
          );
        }
        ...
        const response = await DeductionService.list(
          filter,
          getters.orderBy,
          null,
          null,
