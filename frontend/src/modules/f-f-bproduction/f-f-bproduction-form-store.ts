  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await FFBproductionService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await FFBproductionService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.fFBproduction.create.success'),
        );
        routerAsync().push('/f-f-bproduction');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await FFBproductionService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.fFBproduction.update.success'),
        );
        routerAsync().push('/f-f-bproduction');
      } catch (error) {
        Errors.handle(error);
