...
import fFBproductionListExporterFields from '@/modules/f-f-bproduction/f-f-bproduction-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !fFBproductionListExporterFields ||
          !fFBproductionListExporterFields.length
        ) {
          throw new Error(
            'fFBproductionListExporterFields is required',
          );
        }
        ...
        const response = await FFBproductionService.list(
          filter,
          getters.orderBy,
          null,
          null,
