...
import ffbListExporterFields from '@/modules/ffb/ffb-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !ffbListExporterFields ||
          !ffbListExporterFields.length
        ) {
          throw new Error(
            'ffbListExporterFields is required',
          );
        }
        ...
        const response = await FfbService.list(
          filter,
          getters.orderBy,
          null,
          null,
