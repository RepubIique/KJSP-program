...
import locationListExporterFields from '@/modules/location/location-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !locationListExporterFields ||
          !locationListExporterFields.length
        ) {
          throw new Error(
            'locationListExporterFields is required',
          );
        }
