  ...
  actions: {
    async doInit({ commit }, id) {
      try {
        ...
        if (id) {
          record = await PlantedAreaService.find(id);
        }
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await PlantedAreaService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(
          i18n('entities.plantedArea.create.success'),
        );
        routerAsync().push('/planted-area');
      } catch (error) {
        Errors.handle(error);
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await PlantedAreaService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(
          i18n('entities.plantedArea.update.success'),
        );
        routerAsync().push('/planted-area');
      } catch (error) {
        Errors.handle(error);
