...
import plantedAreaListExporterFields from '@/modules/planted-area/planted-area-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !plantedAreaListExporterFields ||
          !plantedAreaListExporterFields.length
        ) {
          throw new Error(
            'plantedAreaListExporterFields is required',
          );
        }
        ...
        const response = await PlantedAreaService.list(
          filter,
          getters.orderBy,
          null,
          null,
