...
import workCodeListExporterFields from '@/modules/work-code/work-code-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !workCodeListExporterFields ||
          !workCodeListExporterFields.length
        ) {
          throw new Error(
            'workCodeListExporterFields is required',
          );
        }
        ...
        const response = await WorkCodeService.list(
          filter,
          getters.orderBy,
          null,
          null,
