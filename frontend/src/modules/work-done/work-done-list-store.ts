...
import workDoneListExporterFields from '@/modules/work-done/work-done-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !workDoneListExporterFields ||
          !workDoneListExporterFields.length
        ) {
          throw new Error(
            'workDoneListExporterFields is required',
          );
        }
