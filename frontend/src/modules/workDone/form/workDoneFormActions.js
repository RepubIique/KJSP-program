...
const actions = {
  ...
  doCreate: (values) => async (dispatch) => {
    try {
      ...
      await WorkDoneService.create(values);

      dispatch({
        type: actions.CREATE_SUCCESS,
      });

      Message.success(i18n('entities.workDone.create.success'));
  ...
  doUpdate: (id, values) => async (
    ...
    try {
      ...
      await WorkDoneService.update(id, values);

      dispatch({
        type: actions.UPDATE_SUCCESS,
      });

      Message.success(i18n('entities.workDone.update.success'));
