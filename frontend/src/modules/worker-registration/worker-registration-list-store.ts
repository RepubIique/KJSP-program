...
import workerRegistrationListExporterFields from '@/modules/worker-registration/worker-registration-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !workerRegistrationListExporterFields ||
          !workerRegistrationListExporterFields.length
        ) {
          throw new Error(
            'workerRegistrationListExporterFields is required',
          );
        }
        ...
        const response = await WorkerRegistrationService.list(
          filter,
          getters.orderBy,
          null,
          null,
