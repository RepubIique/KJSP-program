...
import blocksSelectors from 'modules/blocks/blocksSelectors';
import destroyActions from 'modules/blocks/destroy/blocksDestroyActions';
import destroySelectors from 'modules/blocks/destroy/blocksDestroySelectors';
import actions from 'modules/blocks/list/blocksListActions';
import selectors from 'modules/blocks/list/blocksListSelectors';
...
class BlocksToolbar extends Component {
  state = {
    ...
    return (
      <ToolbarWrapper>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Button
            component={Link}
            to="/audit-logs?entityNames=blocks"
...
function select(state) {
  return {
    ...
    hasPermissionToEdit: blocksSelectors.selectPermissionToEdit(
      state,
    ),
    hasPermissionToDestroy: blocksSelectors.selectPermissionToDestroy(
      state,
    ),
    hasPermissionToCreate: blocksSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: blocksSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(BlocksToolbar);
