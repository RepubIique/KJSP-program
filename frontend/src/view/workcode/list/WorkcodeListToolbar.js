...
class WorkcodeToolbar extends Component {
  state = {
    ...
    return (
      <ToolbarWrapper>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Button
            component={Link}
            to="/audit-logs?entityNames=workcode"
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: workcodeSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: workcodeSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(WorkcodeToolbar);
